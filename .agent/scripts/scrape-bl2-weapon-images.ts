...
const IMAGE_DIR = join(WEAPONS_DIR, "img");
...
async function fetchInfoboxImageAttributes(title: string): Promise<Record<string, string>> {
  ...
  const figureMatch = html.match(/<figure[^>]*(?:data-source=["'](?:img|image)["']|class=["'][^"']*pi-image[^"']*["'])[^>]*>[\s\S]*?<img\b([^>]*?)>/i);
  if (figureMatch?.[1]) {
...
async function convertToPng(imageBuffer: Buffer): Promise<Buffer> {
  const basePath = join(tmpdir(), `borderlens-bl2-${randomUUID()}`);
  ...
  try {
    ...
    throw new Error(`PNG conversion failed via sips (${reason})`);
  } finally {
...
async function loadWeaponItems(limit?: number): Promise<WeaponItem[]> {
  const entries = await readdir(WEAPONS_DIR, { withFileTypes: true });
  ...
  for (const fileName of chosenFiles) {
    const raw = await readFile(join(WEAPONS_DIR, fileName), "utf-8");
    items.push(JSON.parse(raw) as WeaponItem);
  }
...
async function main(): Promise<void> {
  ...
  const items = await loadWeaponItems(limit);
  ...
  await runWithConcurrency(items, concurrency, async item => {
    ...
    const expectedImagePath = `/img/games/borderlands2/weapons/${item.slug}.png`;
