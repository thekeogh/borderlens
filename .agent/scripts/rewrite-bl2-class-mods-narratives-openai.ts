...
const CLASS_MODS_DIR = path.resolve("data/games/borderlands2/class-mods");
const MAX_CHARS = 600;
...
async function rewriteNarrative(name: string, input: RewriteResult): Promise<RewriteResult> {
  const system = [
    "You rewrite Borderlands class mod text for a game database.",
    "Use British English.",
  ...
  const user = JSON.stringify({ classMod: name, input });
  ...
  const compactSystem = [
    "You compress rewritten class mod text while preserving important gameplay meaning.",
  ...
  const compactRaw = await callOpenAI([
    ...
    { role: "user", content: JSON.stringify({ classMod: name, input: cleaned }) },
  ]);
...
function readClassMod(file: string): ClassMod {
  return JSON.parse(fs.readFileSync(file, "utf8")) as ClassMod;
}

function writeClassMod(file: string, classMod: ClassMod): void {
  fs.writeFileSync(file, `${JSON.stringify(classMod, null, 2)}\n`);
}
...
async function main() {
  ...
  console.log(`Rewriting ${files.length} BL2 class mod files with ${MODEL}...`);
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const fullPath = path.join(CLASS_MODS_DIR, file);
    try {
      const classMod = readClassMod(fullPath);
      const rewritten = await rewriteNarrative(classMod.name || file.replace(/\.json$/, ""), {
        description: classMod.description || "",
        notes: classMod.notes || "",
        specialDescription: classMod.special?.description || "",
      });

      classMod.description = rewritten.description;
      classMod.notes = rewritten.notes;

      const specialTitle = (classMod.special?.title || "").trim();
      if (rewritten.specialDescription) {
        ...
        delete classMod.special;
      }

      writeClassMod(fullPath, classMod);
      updated++;
      console.log(
        `[${String(i + 1).padStart(3, "0")}/${files.length}] ${file} ok ` +
          `(d:${classMod.description.length} n:${(classMod.notes || "").length} s:${classMod.special?.description?.length || 0})`,
      );
    } catch (error) {
      failed++;
