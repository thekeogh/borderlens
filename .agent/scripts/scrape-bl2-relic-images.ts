...
const RELICS_DIR = join(process.cwd(), "data/games/borderlands2/relics");
const IMAGE_DIR = join(RELICS_DIR, "img");
const REPORT_DIR = join(process.cwd(), ".agent/bl2/relics");
...
async function convertToPng(imageBuffer: Buffer): Promise<Buffer> {
  const basePath = join(tmpdir(), `borderlens-bl2-relics-${randomUUID()}`);
...
async function loadRelicItems(limit?: number): Promise<RelicItem[]> {
  const entries = await readdir(RELICS_DIR, { withFileTypes: true });
  ...
  for (const fileName of chosenFiles) {
    const raw = await readFile(join(RELICS_DIR, fileName), "utf-8");
    items.push(JSON.parse(raw) as RelicItem);
  }
...
async function main(): Promise<void> {
  ...
  const items = await loadRelicItems(limit);
  ...
  await runWithConcurrency(items, concurrency, async item => {
    ...
    const expectedImagePath = `/img/games/borderlands2/relics/${item.slug}.png`;
