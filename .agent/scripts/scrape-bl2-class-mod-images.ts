...
const CLASS_MODS_DIR = join(process.cwd(), "data/games/borderlands2/class-mods");
const IMAGE_DIR = join(CLASS_MODS_DIR, "img");
const REPORT_DIR = join(process.cwd(), ".agent/bl2/class-mods");
...
async function convertToPng(imageBuffer: Buffer): Promise<Buffer> {
  const basePath = join(tmpdir(), `borderlens-bl2-class-mods-${randomUUID()}`);
...
async function loadClassModItems(limit?: number): Promise<ClassModItem[]> {
  const entries = await readdir(CLASS_MODS_DIR, { withFileTypes: true });
  ...
  const items: ClassModItem[] = [];

  for (const fileName of chosenFiles) {
    const raw = await readFile(join(CLASS_MODS_DIR, fileName), "utf-8");
    items.push(JSON.parse(raw) as ClassModItem);
  }
...
async function main(): Promise<void> {
  ...
  const items = await loadClassModItems(limit);
  ...
  await runWithConcurrency(items, concurrency, async item => {
    ...
    const expectedImagePath = `/img/games/borderlands2/class-mods/${item.slug}.png`;
