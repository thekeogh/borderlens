...
const SHIELDS_DIR = join(process.cwd(), "data/games/borderlands2/shields");
const IMAGE_DIR = join(SHIELDS_DIR, "img");
const REPORT_DIR = join(process.cwd(), ".agent/bl2/shields");
...
async function convertToPng(imageBuffer: Buffer): Promise<Buffer> {
  const basePath = join(tmpdir(), `borderlens-bl2-shields-${randomUUID()}`);
...
async function loadShieldItems(limit?: number): Promise<ShieldItem[]> {
  const entries = await readdir(SHIELDS_DIR, { withFileTypes: true });
  ...
  for (const fileName of chosenFiles) {
    const raw = await readFile(join(SHIELDS_DIR, fileName), "utf-8");
    items.push(JSON.parse(raw) as ShieldItem);
  }
...
async function main(): Promise<void> {
  ...
  const items = await loadShieldItems(limit);
  ...
  await runWithConcurrency(items, concurrency, async item => {
    ...
    const expectedImagePath = `/img/games/borderlands2/shields/${item.slug}.png`;
